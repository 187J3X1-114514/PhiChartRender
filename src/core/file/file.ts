import JSZip, { OutputType, JSZipObject } from 'jszip'
...
export class File {
    public name: string
    private file: JSZip.JSZipObject | Blob
    ...
    constructor(file: JSZip.JSZipObject | Blob, name?: string) {
    ...
    private async load(): Promise<void> {
        if ((this.file as any).async) {
            this.blob = await (this.file as JSZipObject).async('blob')
    ...
    async async<T extends OutputType>(type: T): Promise<OutputByType[T]> {
        if ((this.file as any).async) {
            return await (this.file as JSZipObject).async(type)
        } else {
