...
export class ResourcePack {
    ...
    static async load(zip: Zip) {
        ...
        log.info('所有Note加载完成')
        const noteSound = {
            tap: await WAudio.from(await (await zip.get(info.sound.tap!)?.getBlob())!.arrayBuffer(), false),
            drag: await WAudio.from(await (await zip.get(info.sound.drag!)?.getBlob())!.arrayBuffer(), false),
            flick: await WAudio.from(await (await zip.get(info.sound.flick!)?.getBlob())!.arrayBuffer(), false),
            hold: await WAudio.from(await (await zip.get(info.sound.hold!)?.getBlob())!.arrayBuffer(), false)
        }
        ...
        log.info('打击效果加载完成')
        const assets = {
            judgeLine: judgeLine, note: note, sound: noteSound, hitFx: textureList
        } as PhiAssets
        console.log(assets)
