...
const GlobalAudioCtx:AudioContext = (new Audio().canPlayType('audio/ogg') == '') ? new oggmentedAudioContext(GlobalAudioCtxConfig) : new AudioCtx(GlobalAudioCtxConfig as AudioContextOptions);
GlobalAudioCtx.addEventListener('statechange', () =>
{
    if (GlobalAudioCtx.state === 'running')
    {
        log.info('Resume AudioContext success');
...
export default class WAudio
{
    ...
    private _buffer: AudioBufferSourceNode | undefined;
    private _timer: AudioTimer | undefined;
    constructor(src:any, loop = false, offset = 0, volume = 1, speed = 1, onend = undefined)
    {
        this.source = src;
        this.loop = loop;
        this.onend = onend;
    ...
    static from(src:any, loop:any):Promise<WAudio>
    {
    ...
    play(withTimer = false)
    {
        ...
        if (this._timer)
        {
            ...
            (this._timer.start as any)(GlobalAudioCtx.currentTime);
        }
        else
        {
...
function parseAudio(arrayBuffer:any)
{
    if (!detectIfIsMp3(arrayBuffer)) return { startOffset: 19 };

    let mp3Tags = (Mp3Parser as any).readTags(new DataView(arrayBuffer));
...
function detectIfIsMp3(arrayBuffer:any)
{
...
function predictMp3Offset(tags:any)
{
    const printWarn = (msg:any) => log.warn('Cannot predict MP3 offset:', msg);
...
{
    log.info('Trying resume AudioContext...');
