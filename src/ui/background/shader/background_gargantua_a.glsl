...
#define ITERATIONS 120         //Increase for less grainy result

const vec3 MainColor = vec3(1.0);
const vec2 iResolution = vec2(1.0);
...
float noise(vec3 x) {
    ...
    vec2 rg = texture(noiseTex, (uv + 0.5) / 256.0, 0.0).yx;
...
void GasDisc(inout vec3 color, inout float alpha, vec3 pos) {
    ...
    dustColor *= pow(texture(iChannel0, radialCoords.yx * vec2(0.15, 0.27)).rgb, vec3(2.0)) * 4.0;
...
vec4 mainImage_0(vec2 fragCoord) {
...
void main() {
    gl_FragColor = mainImage_0(vTextureCoord);
}