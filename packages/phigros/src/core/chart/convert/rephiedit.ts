...
export default function RePhiEditChartConverter(_chart: RPEChart) {
    ...
    rawChart.judgeLineList.forEach((_judgeline: RPEJudgeLine, judgelineIndex: any) => {
        ...
        _judgeline.eventLayers.forEach((_eventLayer: RPEEventLayer | null) => {
                ...
                if (eventName != 'speedEvents') {
                        ...
                        utils.calculateEventEase(event, Easing)
                            .forEach((newEvent) => {
                                switch (eventName) {
                                    case 'moveXEvents':
                                        {
                                            eventLayer.moveXAnim.push(newEvent);
                                            break;
                                        }
                                    case 'moveYEvents':
                                        {
                                            eventLayer.moveYAnim.push(newEvent);
                                            break;
                                        }
                                    case 'alphaEvents':
                                        {
                                            eventLayer.alphaAnim.push(newEvent);
                                            break;
                                        }
                                    case 'rotateEvents':
                                        {
                                            eventLayer.rotateAnim.push(newEvent);
                                            break;
                                        }
                                    default:
                                        {
                    ...
                    _eventLayer.speedEvents.forEach((event: any) => {
                        utils2.separateSpeedEvent(event)
                            .forEach((_event) => {
                                eventLayer.speedAnim.push(_event);
                            }
                            );
                    });
                }
            ...
            if (
                eventLayer.speedAnim.events.length <= 0 &&
                eventLayer.moveXAnim.events.length <= 0 &&
                eventLayer.moveYAnim.events.length <= 0 &&
                eventLayer.alphaAnim.events.length <= 0 &&
                eventLayer.rotateAnim.events.length <= 0
            ) {
                return;
            }
            let speed = utils.calculateRealTime(rawChart.BPMList, eventLayer.speedAnim.events);
            eventLayer.moveXAnim.events = utils.calculateRealTime(rawChart.BPMList, eventLayer.moveXAnim.events);
            eventLayer.moveYAnim.events = utils.calculateRealTime(rawChart.BPMList, eventLayer.moveYAnim.events);
            eventLayer.alphaAnim.events = utils.calculateRealTime(rawChart.BPMList, eventLayer.alphaAnim.events);
            eventLayer.rotateAnim.events = utils.calculateRealTime(rawChart.BPMList, eventLayer.rotateAnim.events);

            speed.forEach((event: any) => {
                event.value = event.value / (0.6 / (120 / 900));
            });
            eventLayer.moveXAnim.events.forEach((event) => {
                event.start = event.start / 1350;
                event.end = event.end / 1350;
            });
            eventLayer.moveYAnim.events.forEach((event) => {
                event.start = event.start / 900;
                event.end = event.end / 900;
            });
            eventLayer.alphaAnim.events.forEach((event) => {
            ...
            eventLayer.rotateAnim.events.forEach((event) => {
                event.start = (Math.PI / 180) * event.start;
                event.end = (Math.PI / 180) * event.end;
            });
            eventLayer.sort();
            eventLayer.moveXAnim.events = fixEvent(eventLayer.moveXAnim.events)
            eventLayer.moveYAnim.events = fixEvent(eventLayer.moveYAnim.events)
            eventLayer.alphaAnim.events = fixEvent(eventLayer.alphaAnim.events)
            eventLayer.rotateAnim.events = fixEvent(eventLayer.rotateAnim.events)
            eventLayer.speedAnim.events = speed
            eventLayer.sort();
    ...
    if (chart.META.RPEVersion <= 100) {
        chart.judgeLineList.forEach((judgeline: any) => {
            ...
            judgeline.eventLayers.forEach((eventLayer: EventLayer) => {
                eventLayer.speedAnim.events.forEach((event: any) => {
                    event.easingLeft = 0;
                    event.easingRight = 1;
                });
                eventLayer.moveXAnim.events.forEach((event: any) => {
                    event.easingLeft = 0;
                    event.easingRight = 1;
                });
                eventLayer.moveYAnim.events.forEach((event: any) => {
                    event.easingLeft = 0;
                    event.easingRight = 1;
                });
                eventLayer.alphaAnim.events.forEach((event: any) => {
                    event.easingLeft = 0;
                    event.easingRight = 1;
                });
                eventLayer.rotateAnim.events.forEach((event: any) => {
                    event.easingLeft = 0;
                    event.easingRight = 1;
                });
