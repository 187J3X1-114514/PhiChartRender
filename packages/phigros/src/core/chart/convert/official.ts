...
export default function OfficialChartConverter(_chart: PhigrosOfficialChart) {
    ...
    rawChart.judgeLineList.forEach((_judgeline, index: number) => {
        ...
        _judgeline.judgeLineMoveEvents.forEach((e: judgeLineMoveEvent) => {
            events.moveXAnim.push({
                startTime: calcRealTime(e.startTime, _judgeline.bpm),
                endTime: calcRealTime(e.endTime, _judgeline.bpm),
                start: e.start - 0.5,
                end: e.end - 0.5
            });
            events.moveYAnim.push({
                startTime: calcRealTime(e.startTime, _judgeline.bpm),
                endTime: calcRealTime(e.endTime, _judgeline.bpm),
                start: e.start2 - 0.5,
                end: e.end2 - 0.5
        ...
        judgeline.sortEvent();
        judgeline.eventLayers[0].moveXAnim.events = utils.arrangeSameValueEvent(judgeline.eventLayers[0].moveXAnim.events);
        judgeline.eventLayers[0].moveYAnim.events = utils.arrangeSameValueEvent(judgeline.eventLayers[0].moveYAnim.events);
        judgeline.eventLayers[0].rotateAnim.events = utils.arrangeSameValueEvent(judgeline.eventLayers[0].rotateAnim.events);
        judgeline.eventLayers[0].alphaAnim.events = utils.arrangeSameValueEvent(judgeline.eventLayers[0].alphaAnim.events);
