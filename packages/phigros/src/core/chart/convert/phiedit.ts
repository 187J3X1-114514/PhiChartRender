...
export default function PhiEditChartConverter(_chart: any) {
    ...
    */
    ...
    for (const lineId in chartSimple._judgelines) {
        ...
        // 事件参数补齐
        judgeline.eventLayers[0].alphaAnim.events.forEach((event: any, eventIndex: any, array: any) => {
        ...
        judgeline.eventLayers[0].moveXAnim.events.forEach((event: any, eventIndex: any, array: any) => {
            if (isNaN(event.endTime)) event.endTime = eventIndex < array.length - 1 ? array[eventIndex + 1].startTime : 1e5;
            if (isNaN(event.start)) event.start = eventIndex > 0 ? array[eventIndex - 1].end : 0;
        });
        judgeline.eventLayers[0].moveYAnim.events.forEach((event: any, eventIndex: any, array: any) => {
            if (isNaN(event.endTime)) event.endTime = eventIndex < array.length - 1 ? array[eventIndex + 1].startTime : 1e5;
            if (isNaN(event.start)) event.start = eventIndex > 0 ? array[eventIndex - 1].end : 0;
        });
        judgeline.eventLayers[0].rotateAnim.events.forEach((event: any, eventIndex: any, array: any) => {
            if (isNaN(event.endTime)) event.endTime = eventIndex < array.length - 1 ? array[eventIndex + 1].startTime : 1e5;
        ...
        judgeline.eventLayers[0].speedAnim.events.forEach((event: any, eventIndex: any, array: any) => {
            if (isNaN(event.endTime)) event.endTime = eventIndex < array.length - 1 ? array[eventIndex + 1].startTime : 1e5;
        });

        // Alpha 事件单独进行计算
        judgeline.eventLayers[0].alphaAnim.events.forEach((event: any) => {
        ...
        judgeline.eventLayers[0].alphaAnim.events = utils.calculateRealTime(chartSimple.bpm, judgeline.eventLayers[0].alphaAnim.events);
        judgeline.eventLayers[0].moveXAnim.events = utils.calculateRealTime(chartSimple.bpm, judgeline.eventLayers[0].moveXAnim.events);
        judgeline.eventLayers[0].moveYAnim.events = utils.calculateRealTime(chartSimple.bpm, judgeline.eventLayers[0].moveYAnim.events);
        judgeline.eventLayers[0].rotateAnim.events = utils.calculateRealTime(chartSimple.bpm, judgeline.eventLayers[0].rotateAnim.events);
        judgeline.eventLayers[0].speedAnim.events = utils.calculateRealTime(chartSimple.bpm, judgeline.eventLayers[0].speedAnim.events);
